"""SMTP Email Tool (Mock Implementation)."""

from typing import Any
from datetime import datetime

from faker import Faker

from app.bigtool.base import BaseEmailTool


fake = Faker()


class SMTPEmail(BaseEmailTool):
    """
    SMTP email tool.
    
    Mock implementation that simulates SMTP email sending.
    In production, this would use smtplib.
    """
    
    def __init__(self):
        super().__init__(
            name="smtp",
            provider="SMTP",
            description="Standard SMTP email",
            is_mock=True,
        )
    
    def _execute(self, params: dict[str, Any]) -> dict[str, Any]:
        """Send email via SMTP (mock)."""
        to_email = params.get("to", params.get("recipient", ""))
        from_email = params.get("from", "noreply@invoice-agent.com")
        subject = params.get("subject", "Invoice Notification")
        
        return {
            "sent": True,
            "message_id": f"<{fake.uuid4()}@invoice-agent.com>",
            "to": to_email,
            "from": from_email,
            "subject": subject,
            "smtp_response": "250 OK",
            "sent_at": datetime.utcnow().isoformat(),
            "provider": self.provider,
        }


__all__ = ["SMTPEmail"]

